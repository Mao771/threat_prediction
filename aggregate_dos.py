...
# Read the PCAP file
def get_traffic_from_s3(file_name, db_helper, plot=True):
    s3 = boto3.client("s3")
    ...
    for pkt in packets:
        if 'IP' in pkt:
            ...
            dt = dt.replace(second=round(dt.second, -1)).strftime("%d-%m-%Y.%H:%M:%S")
    ...
    save_coefs_to_influx(db_helper, coefs, 'ip_packet_coef_10s')
...
def plot_total_coefs(db_helper: DbHelper):
    data = db_helper.get('ip_packet_coef_10s', "", "*")

    sns.relplot(data=data, x="dt", y="coef", kind="line")
    plt.show()
    ewma = EWMADetector(data)
    ewma.detect(traffic_parameter="coef", traffic_parameter_name="Packets to address ratio")
...
if __name__ == '__main__':
    ...
    dt = datetime(year=2024, month=10, day=17, hour=start_hour, minute=start_minute)

    while not (dt.hour == 21 and dt.minute == 33):
        f_name = f'traffc_{str(dt.hour).zfill(2)}{str(dt.minute).zfill(2)}'
        print(f_name)
        get_traffic_from_s3(f_name, db_helper, False)
        dt = dt + timedelta(minutes=1)

    plot_total_coefs(db_helper)
